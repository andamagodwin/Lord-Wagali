    ...
    <Container keyboard className="bg-slate-50">
      <ScrollView className="flex-1 px-4 pt-6">
        <View className="flex-row items-center mb-6">
          ...
          <Text className="text-3xl font-black text-navy-950 uppercase tracking-tighter">Edit Free Tips</Text>
        </View>
        
        {tips.length === 0 ? (
          <View className="bg-white p-8 rounded-[36px] border border-slate-200 items-center justify-center py-12 mb-6">
            <Text className="text-slate-400 text-xs font-bold uppercase tracking-widest text-center">No Active Free Tips</Text>
          </View>
        ) : (
          tips.map((item) => (
            <View key={item.id} className="bg-white p-6 rounded-[36px] mb-4 border border-slate-200 shadow-md">
              <View className="flex-row justify-between items-start mb-4">
                <View className="flex-1 mr-2">
                  ...
                  <Text className="text-coral-500 font-bold text-xs uppercase mt-2">{item.tip} • Odds {item.odds} • {item.prob || '85%'} Prob</Text>
                </View>
                <TouchableOpacity onPress={() => removeFreeTip(item.id)} className="bg-slate-50 p-2.5 rounded-2xl border border-slate-100">
        ...
        <View className="bg-white p-8 rounded-[40px] mt-4 mb-20 border border-slate-200 shadow-md">
           <Text className="text-xl font-black text-navy-950 mb-6 uppercase tracking-tighter">Add Free Tip</Text>
           
           <Text className="text-[10px] font-black text-slate-400 mb-1.5 uppercase tracking-widest px-1">Teams Info</Text>
           <TextInput className="bg-slate-50 p-5 rounded-2xl mb-4 border border-slate-200 font-bold" placeholder="Home Team Name" value={form.home} onChangeText={t => setForm({...form, home: t})} />
           <TextInput className="bg-slate-50 p-5 rounded-2xl mb-4 border border-slate-200 font-bold" placeholder="Away Team Name" value={form.away} onChangeText={t => setForm({...form, away: t})} />
           ...
           <View className="flex-row gap-x-3 mb-4">
             <TextInput className="bg-slate-50 p-5 rounded-2xl border border-slate-200 flex-1 font-bold text-center" placeholder="Home ID" keyboardType="numeric" value={form.homeLogo} onChangeText={t => setForm({...form, homeLogo: t})} />
             <TextInput className="bg-slate-50 p-5 rounded-2xl border border-slate-200 flex-1 font-bold text-center" placeholder="Away ID" keyboardType="numeric" value={form.awayLogo} onChangeText={t => setForm({...form, awayLogo: t})} />
           </View>

           <Text className="text-[10px] font-black text-slate-400 mb-1.5 uppercase tracking-widest px-1">Match Details</Text>
           <TextInput className="bg-slate-50 p-5 rounded-2xl mb-4 border border-slate-200 font-bold" placeholder="League (e.g. Premier League)" value={form.league} onChangeText={t => setForm({...form, league: t})} />
           <TextInput className="bg-slate-50 p-5 rounded-2xl mb-4 border border-slate-200 font-bold" placeholder="Time (e.g. 18:30)" value={form.time} onChangeText={t => setForm({...form, time: t})} />

           <Text className="text-[10px] font-black text-slate-400 mb-1.5 uppercase tracking-widest px-1">Prediction details</Text>
           <TextInput className="bg-slate-50 p-5 rounded-2xl mb-4 border border-slate-200 font-bold" placeholder="Tip (e.g. BTTS or Over 2.5)" value={form.tip} onChangeText={t => setForm({...form, tip: t})} />
           <View className="flex-row gap-x-3 mb-6">
             <TextInput className="bg-slate-50 p-5 rounded-2xl border border-slate-200 flex-1 font-bold text-center" placeholder="Odds (e.g. 1.65)" keyboardType="numeric" value={form.odds} onChangeText={t => setForm({...form, odds: t})} />
             <TextInput className="bg-slate-50 p-5 rounded-2xl border border-slate-200 flex-1 font-bold text-center" placeholder="Prob % (e.g. 85%)" value={form.prob} onChangeText={t => setForm({...form, prob: t})} />
           </View>
